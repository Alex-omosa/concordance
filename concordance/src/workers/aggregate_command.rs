...
// ============ UPDATED WORKER USING REGISTRY DISPATCH ============
...
impl Worker for AggregateCommandWorker {
    ...
    async fn do_work(&self, mut message: AckableMessage<Self::Message>) -> WorkResult<()> {
        ...
        // Step 3: Dispatch to registered aggregate using new registry system
        trace!(
        ...
        // This is the key change - use the new dispatch system instead of manual matching
        let outbound_events = dispatch_command(
            ...
            // Convert from concordance_core::WorkError to local WorkError
            match e {
                concordance_core::WorkError::ConsumerStopped => WorkError::ConsumerStopped,
                concordance_core::WorkError::Fatal(e) => WorkError::Fatal(e),
                concordance_core::WorkError::NatsError(s) => WorkError::NatsError(async_nats::Error::new(async_nats::error::ErrorKind::Other, Some(s))),
                concordance_core::WorkError::Other(s) => WorkError::Other(s),
            }
        })?;
